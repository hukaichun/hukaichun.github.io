...
不行，因為梯度下降的穩定性有上限。對二次目標，$$\eta > 2/\lambda_{\max}$$ 就會發散——這是最佳化課本的內容。

而 $$\lambda_{\max}$$ 怎麼隨寬度走，取決於參數化。在 NTK parameterization 下 $$\hat\Theta \to \Theta_\infty$$，訓練集上的 Gram 矩陣（損失對樣本取平均）最大特徵值收斂到一個跟寬度無關的常數，有限寬的修正是 $$O(n^{-1/2})$$。天花板不隨寬度放寬，而你要的是 $$\sqrt{n}$$ 倍。

寬度相關性並沒有消失，只是搬了家。在實務常用的 standard parameterization 裡，學習率本來就得跟著 $$1/n$$ 縮，Lee 等人（[2019](https://arxiv.org/abs/1902.06720v4)）給了兩者的換算。NTK parameterization 把那個縮放吸進了 $$1/\sqrt{n}$$——也就是第 2 節付的那筆帳。
